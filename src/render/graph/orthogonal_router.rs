...
pub(crate) fn route_edges_orthogonal(
    ...
    let mut routed: Vec<RoutedEdgeGeometry> = geometry
        ...
        .map(|edge| {
            ...
            let label_position = if edge.label_side.is_some_and(|s| {
                s != crate::engines::graph::algorithms::layered::normalize::LabelSide::Center
            }) {
                edge.label_position
            } else {
            ...
            let (head_label_position, tail_label_position) =
                crate::render::graph::routing::compute_end_labels_for_edge(
    ...
    resolve_forward_td_bt_criss_cross_overlaps(diagram, geometry, &mut routed);
    routed
}

fn resolve_forward_td_bt_criss_cross_overlaps(
    diagram: &Diagram,
    geometry: &GraphGeometry,
...
fn pairwise_parallel_clearance(path_a: &[FPoint], path_b: &[FPoint]) -> Option<f64> {
    const EPS: f64 = 0.5;
