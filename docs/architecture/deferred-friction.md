...
### `prepared.rs` passes through raw parser types for simple diagrams

`PreparedPie`, `PreparedPacket`, `PreparedInfo` wrap types directly from
`mermaid::pie::Pie`, `mermaid::packet::Packet`, `mermaid::info::Info`.
Graph-family and Timeline-family have proper intermediate IR.

**Trigger:** Any simple diagram type gains a compilation step or needs
format-independent IR (e.g., SVG support for pie charts).

**Action:** Introduce family-local IR for that diagram type at that point.
