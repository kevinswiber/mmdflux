...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate a Mermaid module dependency map for the Rust mmdflux crate."
    )
    parser.add_argument(
        "--crate-root",
        default="src/lib.rs",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Collapse module paths to this depth when emitting nodes (default: 1).",
    )
    parser.add_argument(
        "--direction",
        choices=["LR", "RL", "TB", "BT"],
...
def mermaid_node_id(module_path: tuple[str, ...]) -> str:
    return "mod_" + "_".join(module_path)


def mermaid_label(module_path: tuple[str, ...]) -> str:
...
def render_mermaid(
    ...
    nodes_by_top_level: dict[str, list[tuple[str, ...]]] = defaultdict(list)
    for node in sorted(grouped_nodes):
        nodes_by_top_level[node[0]].append(node)

    lines = [
        f"%% Generated by scripts/{Path(__file__).name}",
        f"%% crate-root: {crate_root.as_posix()}",
        f"%% max-depth: {max_depth}",
        f"flowchart {direction}",
    ]

    for top_level in sorted(nodes_by_top_level):
        nodes = nodes_by_top_level[top_level]
        if len(nodes) == 1 and len(nodes[0]) == 1:
            node = nodes[0]
            lines.append(f'    {mermaid_node_id(node)}["{mermaid_label(node)}"]')
            continue

        lines.append(f'    subgraph group_{top_level}["{top_level}"]')
        for node in nodes:
            lines.append(f'        {mermaid_node_id(node)}["{mermaid_label(node)}"]')
        lines.append("    end")

    for source, target in sorted(grouped_edges):
        lines.append(f"    {mermaid_node_id(source)} --> {mermaid_node_id(target)}")
...
def main() -> int:
    ...
    if args.max_depth < 1:
        print("--max-depth must be at least 1", file=sys.stderr)
        return 2

    if not crate_root.exists():
        print(f"crate root does not exist: {crate_root}", file=sys.stderr)
        return 2
    ...
    if args.output:
        ...
        print(
            f"wrote Mermaid dependency map to {output_path} "
            f"({len(modules) - 1} modules discovered)",
            file=sys.stderr,
        )
    else:
