...
use mmdflux::{Diagram, OutputFormat, RenderConfig, render_diagram};
...
fn prepare_graph_diagram(diagram_id: &str, input: &str) -> Diagram {
    let prepared = default_registry()
        ...
        .unwrap_or_else(|error| panic!("failed to prepare {diagram_id} input: {error}"));
    let PreparedDiagram::Graph(graph) = prepared else {
        panic!("{diagram_id} input should prepare a graph payload");
    };
    graph.diagram
}
