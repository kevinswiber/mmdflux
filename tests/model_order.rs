...
//!
//! Tests that source declaration order is used as a tie-breaker during
//! crossing reduction when barycenters are equal.
...
fn layout_and_get_x_positions(nodes: &[&str], edges: &[(&str, &str)]) -> HashMap<String, f64> {
    ...
    for &n in nodes {
        graph.add_node(n, (10.0, 10.0));
    }
    for &(from, to) in edges {
        graph.add_edge(from, to);
    }
    let config = LayoutConfig::default();
    let result = layered::layout(&graph, &config, |_, dims| *dims);

    let mut positions = HashMap::new();
    for (id, rect) in &result.nodes {
        let center_x = rect.x + rect.width / 2.0;
        positions.insert(id.0.clone(), center_x);
    }
    positions
}
...
fn edit_stability_add_sibling_preserves_relative_order() {
    ...
    let after =
        layout_and_get_x_positions(&["A", "B", "C", "D"], &[("A", "B"), ("A", "C"), ("A", "D")]);
    // B should still be before C after adding D
    assert!(
        after["B"] < after["C"],
...
fn edit_stability_relative_order_preserved_across_fan_out_extension() {
    ...
    // B, C, D should maintain their relative order
