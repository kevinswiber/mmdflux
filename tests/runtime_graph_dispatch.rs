...
//! public registry/prepared/runtime workflow.
...
/// Detect, parse, and prepare a graph-family fixture through the public registry.
fn prepare_graph_fixture(path: &str) -> (PreparedDiagram, DiagramFamily) {
    ...
    let prepared = registry
        ...
        .unwrap_or_else(|e| panic!("failed to prepare {path}: {e}"));
    (prepared, family)
}
...
#[test]
fn flowchart_and_class_prepare_to_the_same_graph_family_contract() {
    let (fc_prepared, fc_family) = prepare_graph_fixture("tests/fixtures/flowchart/simple.mmd");
    let (class_prepared, class_family) = prepare_graph_fixture("tests/fixtures/class/simple.mmd");

    assert_eq!(fc_family, DiagramFamily::Graph);
    assert_eq!(class_family, DiagramFamily::Graph);

    assert!(matches!(fc_prepared, PreparedDiagram::Graph(_)));
    assert!(matches!(class_prepared, PreparedDiagram::Graph(_)));
}
