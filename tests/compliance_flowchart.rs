...
fn flowchart_fixtures_parse_prepare_via_registry() {
    ...
    for fixture in list_flowchart_fixtures() {
        ...
        let prepared = instance
            ...
            .unwrap_or_else(|e| panic!("Failed to prepare flowchart fixture {fixture}: {e}"));
        assert!(
            matches!(prepared, mmdflux::prepared::PreparedDiagram::Graph(_)),
            "flowchart fixtures should prepare graph payloads: {fixture}"
        );
    }
}
