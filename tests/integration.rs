...
#[test]
fn flowchart_registry_prepare_returns_graph_payload() {
    ...
    let prepared = default_registry()
        ...
        .expect("flowchart fixture should prepare");
    ...
    assert!(graph.diagram.nodes.contains_key("A"));
    assert!(graph.diagram.nodes.contains_key("B"));
    assert!(!graph.diagram.edges.is_empty());
}
